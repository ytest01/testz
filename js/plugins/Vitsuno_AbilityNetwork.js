...
//--(更新情報)------------------------------------------------------------------
// ver1.08 : グレー表示のアイコンの表示が更新されない問題を修正。
// ver1.07 : アビリティをプラグインコマンドから習得する機能を追加。
// ver1.06 : アビリティ表示の実装を修正。
// ver1.05 : 継承元のクラスを定義していたため修正。
// ver1.04 : 職業間共有に対応。
...
var Vitsuno = Vitsuno || {};
Vitsuno.AbilityNet = {};
Vitsuno.AbilityNet.version = 1.08;
 ...
 * @default 64
 * 
 * @requiredAssets img/system/AbilityNetworkLearningMark
 *
 * @help
 * 
 * 必要なプラグイン:
 ...
 *
 * プラグインコマンド:
 *    AbilityNetwork GainActorPoint アクターID 上昇値         # ポイントを増加
 *    AbilityNetwork LearnActorAbility アクターID アビリティID # アビリティを習得
 *    AbilityNetwork ClearActorAbility アクターID            # 習得状態をクリア
 *
...
// ● アビリティを習得する
Game_Actor.prototype.learnAbilityNetwork = function(itemId) {
    var item = $dataArmors[itemId];
    if (DataManager.isAbilityNetwork(item)) {
        // 習得に必要なアビリティポイントも取得
        var value = item.abilityNetPoint - this.acquiredAbilityNetPoint(item.id);
        this.gainAbilityNetPoint(value);
        this.setAbilityNetwork(item.id, item.abilityNetPoint);
    }
}
    ...
    if (command === 'AbilityNetwork') {
        switch (args[0]) {
            case 'GainActorPoint':
                ...
                break;
            case 'LearnActorAbility':
                var actor = $gameActors.actor(Number(args[1]));
                if (actor) {
                    actor.learnAbilityNetwork(Number(args[2]));
                }
                break;
...
// ● 余白の標準値の取得
Perch_AbilityNetwork.prototype.standardPadding = function() {
...
Perch_AbilityNetwork.prototype.drawItem = function(index) {
    ...
    if (item && sprite) {
        ...
        if (item.abilityNetX !== undefined && item.abilityNetY !== undefined) {
            ...
            var mw = this.width - this.padding * 2 - bw;
            var mx = mw * (stage.x + 1) / (stage.size + 1) + this.padding;
            var my = stage.y * this.stageHeight() + this.padding + bh / 2;
            sprite.move(mx, my);
        }
        // 有効表示の設定
...
// ● 項目をクリア
Perch_AbilityNetwork.prototype.clearItem = function(index) {
    var sprite = this.selectableSprite(index);
    if (sprite) {
